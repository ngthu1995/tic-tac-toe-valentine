...
import { TextField, Container, Collapse, ListItem, ListItemText, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Button } from "@material-ui/core";
    ...
    return (
      ...
      >
        ...
        <DialogContent>
          ...
          {
            dialogState.img && <img src={dialogState.img} alt="question" style={{minWidth: 500, paddingLeft: 16, paddingRight: 16}}/>
          }
          <ListItem button onClick={() => setShowSpin(!showSpin)} style={{overflow: 'auto'}}>
            <Collapse in={showAnswer} timeout="auto" unmountOnExit>
            {(dialogState.answer || []).map((text, i) => {
              return (
                <ListItemText component="div" key={i} primary={text} />
              )
            })}
            </Collapse>
          </ListItem >
